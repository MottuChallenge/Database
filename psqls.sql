...
-- Fun��o 1: Converter Dados de uma Vaga (Spot) em JSON
...
-- Execu��o com exce��o NO_DATA_FOUND
SELECT FN_SPOT_PARA_JSON('id_inexistente') FROM dual;


-- Fun��o 2: Valida��o de Placa de Moto
...
BEGIN
    ...
    -- For�a NO_DATA_FOUND se n�o houver registros
    IF v_count = 0 THEN
        RAISE NO_DATA_FOUND;
    END IF;
...
EXCEPTION
    ...
    WHEN VALUE_ERROR THEN
        DBMS_OUTPUT.PUT_LINE('{"erro": "Erro de convers�o de valor"}');
    WHEN OTHERS THEN
...
-- Procedimento 2: A procedure contagem_motos_setor tem como objetivo gerar um relat�rio manual de contagem de motos por setor dentro do p�tio, exibindo tamb�m o total geral de motos no p�tio.
...
BEGIN
    ...
    FOR rec IN c_spots LOOP
        ...
        EXCEPTION
            ...
            WHEN VALUE_ERROR THEN
                DBMS_OUTPUT.PUT_LINE('Erro: valor inv�lido encontrado em num_motos');
            WHEN OTHERS THEN
...
BEGIN
    ...
    -- Inser��o na tabela de auditoria
